...
@CommonsLog
public class SimpleHostRoutingFilter2 extends ZuulFilter {
	...
	private final Runnable clientloader = new Runnable() {
		...
		public void run() {
			try {
				SimpleHostRoutingFilter2.this.httpClient.close();
			}
			catch (IOException ex) {
				log.error("error closing client", ex);
			}
			SimpleHostRoutingFilter2.this.httpClient = newClient();
		}
	};

	public SimpleHostRoutingFilter2(ProxyRequestHelper helper, ZuulProperties properties) {
		this.helper = helper;
	...
	private void initialize() {
		...
		this.connectionManagerTimer.schedule(new TimerTask() {
			@Override
			public void run() {
				if (SimpleHostRoutingFilter2.this.connectionManager == null) {
					return;
				}
				SimpleHostRoutingFilter2.this.connectionManager.closeExpiredConnections();
			}
		}, 30000, 5000);
	}
